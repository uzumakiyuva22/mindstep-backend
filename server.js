...
const JD_JAVA_VERSION_INDEX = process.env.JDOODLE_JAVA_VERSION_INDEX || "";
...
async function runOnJDoodle(language, versionIndex, script) {
  if (!JD_CLIENT_ID || !JD_CLIENT_SECRET) return { error: "JDoodle not configured" };
...
async function runOnPiston(language, version, files) {
  try {
    ...
    const out =
      data.run?.stdout || data.run?.output || data.run?.stderr || JSON.stringify(data);
    return { output: out };
  } catch (e) {
    return { error: e.message || String(e) };
  }
...
function adminAuth(req, res, next) {
  if (!ADMIN_SECRET)
    return res.status(403).json({ error: "Admin routes disabled" });
...
// Run code — JDoodle first, fallback Piston
app.post("/run-code", async (req, res) => {
  try {
    ...
    if (JD_CLIENT_ID && JD_CLIENT_SECRET) {
      if (language === "java") {
        const jv = JD_JAVA_VERSION_INDEX || "0";
        ...
        const remote = await runOnPiston("java", "21", [
          { name: "Main.java", content: source },
        ]);
        return res.json({ output: remote.output });
      }

      if (language === "python") {
        const runResult = await runOnJDoodle("python3", "3", source);

        if (runResult.output) return res.json({ output: runResult.output });

        const remote = await runOnPiston("python", "3.10.0", [
          { name: "script.py", content: source },
        ]);
        return res.json({ output: remote.output });
      }
      ...
      if (language === "javascript") {
        ...
        const remote = await runOnPiston("javascript", "18.15.0", [
          { name: "script.js", content: source },
        ]);
        return res.json({ output: remote.output });
      }
    }
